...
function s:RubyWarning()
  echohl WarningMsg
  echo "Synbad requires Vim to be compiled with Ruby support!"
  echohl none
endfunction

function Synbad()
  if has('ruby')
    if filereadable(".syntax")
      ruby <<EOF
      #require 'synbad'
      #require File.join(File.dirname(__FILE__), '../ruby/synbad')
